...
ICP_OUT icp(const MatrixXd &A, const MatrixXd &B, int max_iteration, double tolerance) {
    size_t rows_A = A.rows();
    size_t rows_B = B.rows();

    MatrixXd src = MatrixXd::Ones(4, rows_A);  // Source matrix
    MatrixXd src3d = MatrixXd::Ones(3, rows_A);  // Source matrix (3D)
    MatrixXd tgt3d = MatrixXd::Ones(3, rows_A);  // Nearest targets
    MatrixXd T;
    MatrixXd Trans = MatrixXd::Identity(4, 4);  // initialize to identity
    NEIGHBOR neighbor;
    ICP_OUT result;
    int iter = 0;
    ...
    for (iter = 0; iter < max_iteration; iter++) {
        ...
        for (size_t i = 0; i < rows_A; i++)
            tgt3d.block<3, 1>(0, i) = B.block<1, 3>(neighbor.indexes[i], 0).transpose();
        ...
        cout << "calculate error difference: ";
    ...
    result.trans = Trans;  // Use accumulated transform
